...
<script>
   ...
   $(function() {
        // Inicializar Select2 en todos los campos relevantes
        $('#pais_origen, #estado_origen, #municipio_origen, #pais_destino, #estado_destino, #municipio_destino').select2({ width: '100%' });
    ...
    $('#pais_origen').on('change', function() {
        ...
        if (String(idPais) === String(142)) { // México
            $('#wrap_estado_select').show();
            $('#wrap_estado_text').hide();
            // CORRECCIÓN: Llenar el select de estados de origen
    ...
    // --- LÓGICA PAÍS DESTINO (CORREGIDA) ---
    $('#pais_destino').on('change', function() {
        const idPaisDestino = $(this).val();
        if (String(idPaisDestino) === String(142)) { // México
            $('#destino_estado_select').show();
            $('#destino_estado_text').hide().prop('disabled', true);
            $('#estado_destino').prop('disabled', false);
            // AQUÍ FALTABA: Llenar el select de estados de destino
            llenarSelect($('#estado_destino'), CAT_ESTADOS, 'id_estado', 'dsc_estado', 'Seleccione estado');
        } else {
            $('#destino_estado_select').hide();
            $('#destino_estado_text').show().prop('disabled', false);
            $('#estado_destino').prop('disabled', true);
        }
    });

    // --- LÓGICA ESTADO → MUNICIPIO ORIGEN ---
    $('#estado_origen').on('change', function() {
        const idEstado = $(this).val();
        if (idEstado) {
            $('#wrap_municipio_text').hide();
            $('#wrap_municipio_select').show();
            // Filtramos municipios por el estado seleccionado
            const municipiosFiltrados = CAT_MUNICIPIOS.filter(m => String(m.id_estado) === String(idEstado));
            llenarSelect($('#municipio_origen'), municipiosFiltrados, 'id_municipio', 'nombre_municipio', 'Seleccione municipio');
        }
    });

    // --- LÓGICA ESTADO → MUNICIPIO DESTINO (CORREGIDA) ---
    $('#estado_destino').on('change', function() {
        const idEstado = $(this).val();
        if (idEstado) {
            $('#destino_municipio_text').hide();
            $('#destino_municipio_select').show();
            // CORRECCIÓN: Filtrar y llenar municipios de destino
            const municipiosFiltrados = CAT_MUNICIPIOS.filter(m => String(m.id_estado) === String(idEstado));
            llenarSelect($('#municipio_destino'), municipiosFiltrados, 'id_municipio', 'nombre_municipio', 'Seleccione municipio');
        }
        });

        // Inicialización automática
        $('#pais_origen').trigger('change');
        $('#pais_destino').trigger('change');
    });
