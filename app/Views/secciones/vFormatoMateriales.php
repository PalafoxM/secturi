        ...
        <div class="container-fluid">
            ...
            <div class="container-fluid" style="background: white; padding: 20px;">
                ...
                <form id="formPagoTerceros" enctype="multipart/form-data">
                    ...
                    <input type="hidden" name="tipo_formato" id="tipo_formato" value="MATERIALES">
                                    ...
                                    <div class="d-flex align-items-center justify-content-center">
                                        <select id="folio" name="folio" class="folio">
                                            <?php foreach($cat_area as $area): ?>
                                                <option value="<?= $area->id_area ?>" <?= $area->id_area == 25 ? 'selected' : '' ?>><?= $area->prefijo ?></option>
                                            <?php endforeach; ?>
                                        </select>
                            ...
                            <tr>
                                <td colspan="4" class="text-left bg-white" style="border-bottom: none !important;">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <input type="text" name="no_convenio"  class="form-control-plaintext text-left w-50" value="<?= isset($registro_pt->no_convenio) ? $registro_pt->no_convenio : '' ?>">
                                    </div>
                                    <div class="d-flex align-items-center">
                            ...
                            <tr style="height: 100px;">
                                ...
                                <td class="align-bottom pb-3">
                                    <select name="nombre_autoriza" id="nombre_autoriza">
                                        ...
                                        <?php foreach ($usuarios as $usuario): ?>
                                            <?php if(in_array($usuario->id_usuario, [95, 105, 18, 34])): ?>
                                ...
                                <td class="align-bottom pb-3">
                                    <select name="nombre_responsable_1" id="nombre_responsable_1">
                                        <option value="">Seleccione una opción</option>
                                        <?php foreach ($usuarios as $usuario): ?>
                                            <?php if(in_array($usuario->id_usuario, [152, 40, 105,18, 99, 120,34 ])): ?>
                                            ...
                                            <?php endif ?>
                                        <?php endforeach; ?>
                                         <option value="NO APLICA" <?= isset($registro_pt->nombre_responsable) ? ($registro_pt->nombre_responsable == 'NO APLICA' ? 'selected' : '') : 'selected' ?>>NO APLICA</option>
                                    </select>

                                    <input type="text" name="cargo_responsable_1" class="form-control-plaintext small" value="<?= isset($registro_pt->cargo_responsable) ? $registro_pt->cargo_responsable : 'NO APLICA' ?>">
                                </td>
                            </tr>
                             <tr>
                                ...
                                <td class="align-bottom pb-3 border" style="height: 100px;">
                                     <select name="nombre_responsable_2" id="nombre_responsable_2">
                                        ...
                                        <?php foreach ($usuarios as $usuario): ?>
                                            ...
                                            <option value="<?= $usuario->nombre_completo ?>" <?= isset($registro_pt->nombre_responsable_2) ? ($registro_pt->nombre_responsable_2 == $usuario->nombre_completo ? 'selected' : '') : ($usuario->nombre_completo == 'ELIZABETH CRISTINA MONDRAGON MARTINEZ' || $usuario->nombre_completo == 'ELIZABETH CRISTINA MONDRAGÓN MARTÍNEZ' ? 'selected' : '') ?>><?= $usuario->nombre_completo ?></option>
                                    ...
                                    <input type="text" name="cargo_responsable_2" class="form-control-plaintext small" value="<?= isset($registro_pt->cargo_responsable_2) ? $registro_pt->cargo_responsable_2 : 'COORDINADORA DE RECURSOS MATERIALES Y SERVICIOS GENERALES' ?>">
                                </td>
                            </tr>
                        </tbody>
                    </table>
      ...
      $(document).on('change', 'input[accept=".xml"]', function(e) {
        ...
        $('#btnGuardarPT').click(function() {
            ...
            $btn.html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Procesando...');
             ...
             $.ajax({
                url: "<?php echo base_url(); ?>index.php/Agregar/guardaFormatoMateriales",
                type: "POST",
                data: formData,
                processData: false,
                contentType: false,
                success: function(data) {
                    if (data.error) {
                        ...
                        Swal.fire({
                            ...
                            window.location.href = "<?php echo base_url(); ?>index.php/Inicio/ListaHojaAzulMateriales";
                        });
                    }
                },
                error: function() {
